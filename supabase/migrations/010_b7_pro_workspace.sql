...
create policy "projects_insert_own" on public.calculation_projects for insert with check(auth.uid()=user_id);
create policy "projects_update_own" on public.calculation_projects for update using(auth.uid()=user_id) with check(auth.uid()=user_id);
...
create policy "scenarios_insert_own" on public.saved_scenarios for insert with check(auth.uid()=user_id and (project_id is null or exists(select 1 from public.calculation_projects p where p.id=project_id and p.user_id=auth.uid())));
create policy "scenarios_update_own" on public.saved_scenarios for update using(auth.uid()=user_id) with check(auth.uid()=user_id and (project_id is null or exists(select 1 from public.calculation_projects p where p.id=project_id and p.user_id=auth.uid())));
