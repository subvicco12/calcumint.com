...
export default async function CustomCalculatorBuilderPage({ params }: PageProps) {
  ...
  const { data: calculator } = await supabase.from("custom_calculators").select("id,organization_id,name,slug,description,status,current_version,published_version,updated_at").eq("id", id).maybeSingle();
    ...
    <section className="container page-top business-page">
      <div className="section-heading">
        <div>
          ...
          <p className="hero-copy">/{calculator.slug} · {calculator.status} · current v{calculator.current_version}{calculator.published_version ? ` · published v${calculator.published_version}` : ""}</p>
        </div>
      ...
      <div className="builder-grid">
        ...
        <article className="card">
          ...
          <ul><li>Arithmetic and powers</li><li>Comparisons and boolean logic</li><li>IF, MIN, MAX, ABS, ROUND, FLOOR, CEIL</li><li>Outputs may reference earlier numeric outputs</li></ul>
        </article>
      </div>
      ...
      <div className="account-grid section">
        <article className="card">
          ...
          <p>Publishing points the calculator at a specific immutable version. Editing later creates another draft version.</p>
