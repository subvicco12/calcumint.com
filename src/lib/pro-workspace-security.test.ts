...
describe("B7 Pro workspace migration",()=>{
 ...
 it("requires a paid plan for workspace inserts",()=>{expect(migration).toContain("p.plan in ('pro','business')");expect(migration).toContain("profile.plan in ('pro','business')");expect((migration.match(/plan in \('pro','business'\)/g)??[]).length).toBeGreaterThanOrEqual(4)});
 it("constrains persisted workspace text",()=>{expect(migration).toContain("char_length(description)<=1000");expect(migration).toContain("char_length(calculator_slug) between 1 and 160")});
