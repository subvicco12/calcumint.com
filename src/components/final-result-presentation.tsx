...
export function FinalResultPresentation({result,plan="free"}:{result:StructuredCalculationResult;plan?:PlanTier}){
  ...
  return <section className="final-result" aria-label="Calculation result">
    ...
    {result.schedule&&result.schedule.length>0&&(hasCalculatorCapability(plan,"detailedSchedule")?<details className="result-methodology"><summary>Detailed schedule</summary><div className="schedule-scroll"><table className="result-table"><thead><tr><th>Period</th>{Object.keys(result.schedule[0].values).map(k=><th key={k}>{k}</th>)}</tr></thead><tbody>{result.schedule.map(row=><tr key={row.id}><td>{row.period}</td>{Object.entries(row.values).map(([k,v])=><td key={k}>{display(v)}</td>)}</tr>)}</tbody></table></div></details>:<div className="locked-feature"><span aria-hidden="true">🔒</span><strong>Detailed schedule</strong><small>Full period-by-period schedule is included with Pro.</small></div>)}
