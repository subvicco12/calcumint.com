...
describe("fixed / term deposit", () => {
  ...
  it("rejects unsafe direct helper inputs",()=>{expect(()=>termDepositFutureValue(Number.POSITIVE_INFINITY,8,12,4)).toThrow();expect(()=>termDepositFutureValue(1000,-1,12,4)).toThrow();expect(()=>termDepositFutureValue(1000,8,12.5,4)).toThrow();expect(()=>termDepositFutureValue(1000,8,12,3)).toThrow();expect(()=>termDepositFutureValue(1000,1001,12,4)).toThrow();expect(()=>termDepositFutureValue(1000,8,1201,4)).toThrow();expect(()=>termDepositFutureValue(Number.MAX_VALUE,1000,1200,365)).toThrow(/numeric range/);});
});
