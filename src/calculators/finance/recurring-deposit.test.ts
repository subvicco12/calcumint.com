...
describe("recurring deposit engine", () => {
  ...
  it("rejects unsafe direct solver inputs",()=>{expect(recurringDepositFutureValue(Number.POSITIVE_INFINITY,7,12)).toBe(0);expect(()=>recurringDepositFutureValue(1000,-1,12)).toThrow();expect(recurringDepositFutureValue(1000,7,12.5)).toBe(0);expect(requiredMonthlyRecurringDeposit(Number.POSITIVE_INFINITY,7,12)).toBe(0);expect(()=>requiredMonthlyRecurringDeposit(100000,-1,12)).toThrow();});
});
