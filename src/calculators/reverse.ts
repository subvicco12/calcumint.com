...
export function goalSeek(target:number,evaluate:(candidate:number)=>number,options:GoalSeekOptions):GoalSeekResult {
  ...
  if(Math.abs(lowError)<=tolerance)return{value:low,achieved:lowValue,target,error:lowError,iterations:0,converged:true};
  if(Math.abs(highError)<=tolerance)return{value:high,achieved:highValue,target,error:highError,iterations:0,converged:true};
  if(direction==="increasing"&&highValue<=lowValue)throw new Error("Declared increasing direction is inconsistent with solver boundary outputs");
  if(direction==="decreasing"&&highValue>=lowValue)throw new Error("Declared decreasing direction is inconsistent with solver boundary outputs");
  const minOutput=Math.min(lowValue,highValue),maxOutput=Math.max(lowValue,highValue);
  ...
  let candidate=low,achieved=lowValue,error=lowError;
  for(let iterations=1;iterations<=maxIterations;iterations+=1){
    ...
    if(moveLow)low=candidate;else high=candidate;
    if(Math.abs(high-low)<=Number.EPSILON*Math.max(1,Math.abs(low),Math.abs(high)))break;
  }
