...
describe("public calculator publication gate", () => {
  it("publishes only certified calculator definitions", () => { const publicItems=listPublicCalculators(); expect(publicItems.length).toBeGreaterThan(0); for(const item of publicItems){const definition=calculatorRegistry.getBySlug(item.slug);expect(definition).toBeDefined();expect(definition?.reviewStatus).toBe("certified");expect(definition?.category).toBe(item.category);} });
  it("requires usable provenance for every public calculator", () => { for(const item of listPublicCalculators()){const definition=calculatorRegistry.getBySlug(item.slug);expect(definition).toBeDefined();expect(definition?.formulas.length).toBeGreaterThan(0);expect(definition?.examples.length).toBeGreaterThan(0);expect(definition?.sources.length).toBeGreaterThan(0);for(const source of definition?.sources??[]){expect(source.label.trim().length).toBeGreaterThan(0);const sourceUrl=source.url;if(sourceUrl)expect(()=>new URL(sourceUrl)).not.toThrow();}} });
  ...
  it("prepares the finance category without making draft finance pages public", () => { const categories=new Set(listPublicCategories().map((category)=>category.slug));expect(categories.has("finance-investment")).toBe(true);expect(categories.has("loans-mortgages")).toBe(true);expect(listPublicCalculators().some((item)=>item.category==="finance-investment")).toBe(false);expect(listPublicCalculators().some((item)=>item.category==="loans-mortgages")).toBe(false); });
});
