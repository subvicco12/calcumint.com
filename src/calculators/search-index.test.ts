...
describe("public calculator search index",()=>{
 it("contains only certified public calculators",()=>{const index=buildPublicCalculatorSearchIndex();expect(index.length).toBeGreaterThan(0);for(const item of index)expect(item.href).toBe(`/calculators/${item.category}/${item.slug}`);expect(index.some(x=>x.slug==="loan-emi-calculator")).toBe(false)});
