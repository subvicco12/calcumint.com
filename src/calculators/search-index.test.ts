...
describe("public calculator search index",()=>{
 ...
 it("uses curated unit-conversion metadata for natural-language routing",()=>{const item=buildPublicCalculatorSearchIndex().find(x=>x.slug==="unit-conversion-calculator");expect(item).toBeDefined();expect(item?.keywords).toContain("convert miles to kilometers");expect(findPublicCalculatorCandidates("convert miles to kilometers")[0]?.slug).toBe("unit-conversion-calculator")});
