...
import {proteinIntakeCalculator,carbohydrateIntakeCalculator,fatIntakeCalculator,caloriesBurnedCalculator,paceCalculator,oneRepMaxCalculator,healthBatch2Definitions} from "./catalog-batch-2";
describe("Health catalog #398-403 draft engines",()=>{
 it("matches golden references",()=>{
  ...
  expect(runCalculator(paceCalculator,{distanceKm:5,timeMinutes:30}).output.value).toBe(6);
 ...
 it("rejects invalid inputs",()=>{
  ...
  expect(()=>runCalculator(paceCalculator,{distanceKm:0,timeMinutes:30})).toThrow();
