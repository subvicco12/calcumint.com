...
describe("country discovery",()=>{
 it("never exposes draft country priorities",()=>{expect(listCertifiedCountryPriorities("IN").some(x=>x.slug==="loan-emi-calculator")).toBe(true);expect(listCertifiedCountryPriorities("IN").some(x=>x.slug==="sip-calculator")).toBe(true)});
