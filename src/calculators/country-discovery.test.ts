...
describe("country discovery",()=>{
 ...
 it("exposes explicit regional defaults without draft leakage",()=>{const india=getCountryDiscoveryProfile("IN");expect(india).toMatchObject({country:"IN",currency:"INR",locale:"en-IN",unitSystem:"metric"});expect(india?.items.some(x=>x.slug==="loan-emi-calculator")).toBe(false)});
});
