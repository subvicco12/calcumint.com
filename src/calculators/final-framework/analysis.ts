...
type LoanAnalysisInput={principal:number;annualRatePercent:number;termMonths:number;extraMonthlyPayment?:number};
export function loanRateSensitivity(input:LoanAnalysisInput,spread=2):SensitivityPoint[]{
 const extraMonthlyPayment=input.extraMonthlyPayment??0;
 return [-spread,-spread/2,0,spread/2,spread].map(d=>{const rate=Math.max(0,input.annualRatePercent+d);return {input:rate,value:runCalculator(loanAnalysisCalculator,{...input,extraMonthlyPayment,annualRatePercent:rate}).output.totalInterest}});
}
export function loanScenarios(input:LoanAnalysisInput):Scenario[]{
 const extraMonthlyPayment=input.extraMonthlyPayment??0;
 const canonical={...input,extraMonthlyPayment};
 const base=runCalculator(loanAnalysisCalculator,canonical).output.totalInterest;
 return [-1,0,1].map((d,i)=>{const rate=Math.max(0,input.annualRatePercent+d);const value=runCalculator(loanAnalysisCalculator,{...canonical,annualRatePercent:rate}).output.totalInterest;return{id:String(i),label:rate.toFixed(2)+"% rate",value,delta:value-base}});
}
